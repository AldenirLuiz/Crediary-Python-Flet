...
class MyTable(Toplevel):
    ...
    def layout_data(self, _table):
        ...
        _column = self._handler.query_request_columns(_table)
        _data = self._handler.request_data(_table)
        dict_data = dict(zip(_column, _data[0]))
        ...
        for card in self.cards:  # percorrendo os dados de layout arquivo cellNames.json
            ...
            if card not in self._exclude_cards:
                Layout.creat_lay(frm, self.cards[card], 'label', data=dict_data)
